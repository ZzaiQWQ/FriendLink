...
public final class BackportedConnectionFactory {
    ...
    private static final Field CHANNEL_FIELD = findField("channel", Channel.class);
    private static final Field ADDRESS_FIELD = findField("address", SocketAddress.class);
    private static final Field BANDWIDTH_FIELD = findField("bandwidthDebugMonitor", BandwidthDebugMonitor.class);
    ...
    private static Field findField(String name, Class<?> type) {
        try {
            ...
            throw new ExceptionInInitializerError(new NoSuchFieldException(name + " or type " + type.getName()));
        }
    }
